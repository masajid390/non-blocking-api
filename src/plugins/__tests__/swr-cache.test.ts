...
describe('swr-cache plugin', () => {
    ...
    it('fetches on cache miss and caches the result', async () => {
        ...
        const res = await (server as FastifyInstance as FastifyInstanceWithConfig).swr('key1', fetcher);
        ...
        const res2 = await (server as any).swr('key1', fetcher);
        expect(res2).toEqual({ value: 1 });
        // background fetch should have been started
        expect(fetcher).toHaveBeenCalledTimes(2);

        // now cached value should be the updated one
        const res3 = await (server as FastifyInstance as FastifyInstanceWithConfig).swr('key1', fetcher);
